...
public interface AccountMetricRepository extends JpaRepository<AccountMetric, Integer> {
	...
	@Query("""
			...
			""")
	List<AccountMetric> getAccountMetricsByAccountKeys(List<BigInteger> accountKey, Date startDate,
			Date endDate, Pageable pageable);

	@Query("""
			SELECT am from AccountMetric am
			WHERE am.accountKey IN ?1
			""")
	List<AccountMetric> getAccountMetricsByAccountKeys(List<BigInteger> accountKey);

	@Query("""
			SELECT am from AccountMetric am
			WHERE am.accountKey IN ?1 and am.snapShotDate =?2
			""")
	List<AccountMetric> getAccountMetricsBySnapShotDate(List<BigInteger> accountKey,
			Date snapShotDate);
}
